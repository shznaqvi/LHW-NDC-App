...
object ServerSecurity {
    ...
    fun encrypt(plain: String, apikey: String): String {
        return try {
            val iv = ByteArray(16)
            val cipher =
                Cipher.getInstance(KeyProperties.KEY_ALGORITHM_AES + "/" + KeyProperties.BLOCK_MODE_CBC + "/" + "PKCS5Padding")
            cipher.init(
                Cipher.ENCRYPT_MODE,
                SecretKeySpec(apikey.toByteArray(StandardCharsets.UTF_8), "AES"),
                IvParameterSpec(iv)
            )
