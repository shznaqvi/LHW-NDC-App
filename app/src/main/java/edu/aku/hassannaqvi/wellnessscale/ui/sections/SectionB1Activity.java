...
import edu.aku.hassannaqvi.wellnessscale.databinding.ActivitySectionB1Binding;
...
public class SectionB1Activity extends AppCompatActivity {
    ...
    private boolean updateDB() {
        ...
        familyMembers.updateRapidScore();
        db = MainApp.appInfo.getDbHelper();
        long updcount = 0;
        try {
            updcount = db.updatesFamilyMembersColumn(TableContracts.FamilyMembersTable.COLUMN_SB1, familyMembers.sB1toString());
    ...
    public void btnContinue() {
        ...
        if (updateDB()) {
            setResult(RESULT_OK);
            Intent i;
            i = new Intent(this, SectionC1Activity.class).putExtra("complete", true).setFlags(Intent.FLAG_ACTIVITY_FORWARD_RESULT);
            startActivity(i);
            finish();
        } else {
